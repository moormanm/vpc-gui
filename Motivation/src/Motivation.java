...
public class Motivation extends JFrame{
	...
	public static abstract class Action {
		public abstract void act(HashMap<String,Float> semanticScores, int numChains);
	}
	...
	public static  void semanticScoreIteration(HashMap<String,Float> semanticScores) {
	...
	public static void eval(Component currentComponent) {
		...
		if(currentField != null) {
		  distScores = calcDistScores(currentField, new LinkedList<String>(fields.keySet()));
		}
		
		HashMap<String,Float> groupScores = calcGroupScores(new LinkedList<String>(fields.keySet()));
		HashMap<String,Float> seqScores = calcSeqScores( new LinkedList<String>(fields.keySet()));
	...
	public static HashMap<String, Float> calcDistScores(String currentField, LinkedList<String> fields) {
	...
	public static HashMap<String, Float> calcGroupScores(LinkedList<String> fieldNames) {
	...
	public static HashMap<String, Float> calcSeqScores(LinkedList<String> fieldNames) {
		...
		for(String groupName : groupMembers.keySet()) {
			...
			for(String member : members) {
				...
				rankedList.add(new Pair<String,Integer>(member, sequenceInGroup.get(member)));
			}
	...
	public static class MyField {
	   ...
	   String value() {
		   ...
		   throw new RuntimeException("logic error in value(): Unknown class type: " + child.getClass());
	   }
	   ...
	   void reset() {
		   ...
		   throw new RuntimeException("logic error in isSet(): Unknown class type: " + child.getClass());
	   ...
	   int getX() {
		   ...
		   return (int)((float)d.width/2.0f + (float)child.getLocationOnScreen().x);
	   }
	   int getY() {
		   Dimension d = child.getSize();
		   return (int)((float)d.height/2.0f + (float)child.getLocationOnScreen().y);
	   }
	}
	...
	public static void paintField(String field, float score) {
		...
		if(panel == null) {
			throw new RuntimeException("Panel does not exist for field " + field);
		}
	...
	public void makeForm() {
		...
		JPanel productsCustomerServiceGroup = makeFieldGroup(
				"productsCustomerServiceGroup", "Please rate the quality of our products and customer service:",
				makeRadioButtonField("QualityOfServiceAtTheBar", "The quality of service at the bar.", "1", "2", "3","4", "5"),
				makeRadioButtonField("QualityOfServiceAtTheRegister", "The quality of service at the Register.", "1", "2", "3","4", "5"),
				makeRadioButtonField("QualityOfServiceMyFoodItems", "The quality of my food items.", "1", "2", "3","4", "5"),
				makeRadioButtonField("QualityOfMyBeverage", "The quality of my beverage.", "1", "2", "3","4", "5"));
		
		JPanel dateTimeGroup = makeFieldGroup("dateTimeGroup", "Please note the date and time of your visit:",
				makeTextField("DateOfVisit", "Date:", 10),
				makeTextField("TimeOfVisit", "Time", 10));


		JPanel cleanlinessGroup = makeFieldGroup("cleanlinessGroup", "How would you rate our cleanliness:",
				makeRadioButtonField("outsideOfStoreCleanliness", "The outside of the store was clean.", "1", "2", "3","4", "5"),
				makeRadioButtonField("barCleanliness", "The bar was clean.", "1", "2", "3","4", "5"),
				makeRadioButtonField("tablesChairsCleanliness", "The tables and chairs were clean.", "1", "2", "3","4", "5"),
				makeRadioButtonField("floorsCleanliness", "The floors were clean.", "1", "2", "3","4", "5"),
				makeRadioButtonField("windowsAndDoorsCleanliness", "The windows and doors were clean.", "1", "2", "3","4", "5"),
				makeRadioButtonField("restroomsCleanliness", "The restrooms were clean.", "1", "2", "3","4", "5"));
		
		JPanel experienceGroup = makeFieldGroup("experienceGroup", "Based on your experience, would you..",
				makeRadioButtonField("comeBackAgain", "Come back again?", "Yes", "No"),
				makeRadioButtonField("reccomendUs", "Recommend us to a friend?", "Yes", "No"),
				makeRadioButtonField("positionWithUs", "be interested in a position with us?", "Yes", "No"));
		
		JPanel generalCommentsGroup = makeFieldGroup("generalCommentsGroup", "Please share any other comments you have about us.",
	...
	public JPanel makeRadioButtonField(String fieldName, String preamble, String ... btnLabels) {
	...
	public static JPanel makeFieldGroup(String groupName, String title, JPanel ... fieldPanelsList) {
		...
		for(JPanel jp : fieldPanelsList) {
			...
			if(fieldName == null) {
				throw new RuntimeException("Could not find panel on panelFields");
			}
	...
	public static void standardBorder(JPanel jp, String name) {
		jp.setBorder(BorderFactory.createTitledBorder(
				...
				TitledBorder.TOP, new FontUIResource(new Font("tahoma", Font.BOLD, 14))));
	...
	public static void buildSemanticRules() {
		Condition c = new Condition() {
			@Override
			public boolean eval() {
				//if the restrooms weren't clean, we really want to know what time they were there
				if(fieldIsSet("restroomsCleanliness") && (!fieldIsSet("DateOfVisit")  || !fieldIsSet("TimeOfVisit"))) {
		...
		Action a = new Action() {
			...
			public void act(HashMap<String, Float> semanticScores, int numChains) {
				...
				semanticScores.put("DateOfVisit", dov+.3f*(numChains*numChains));
				semanticScores.put("TimeOfVisit", tov+.3f*(numChains*numChains));
			}
		};
		semanticMap.put(c, a);
		...
		c = new Condition() {
			@Override
			public boolean eval() {
				//if the bar service was bad, we really want to know what time they were there
				if(fieldIsSet("QualityOfServiceAtTheBar") && (!fieldIsSet("DateOfVisit")  || !fieldIsSet("TimeOfVisit"))) {
		...
		c = new Condition() {
			@Override
			public boolean eval() {
				//if the register service was bad, we really want to know what time they were there
				if(fieldIsSet("QualityOfServiceAtTheRegister") && (!fieldIsSet("DateOfVisit")  || !fieldIsSet("TimeOfVisit"))) {
		...
		c = new Condition() {
			@Override
			public boolean eval() {
				//if they would recommend us to a friend, we really want them to fill out the comments field
				if(fieldIsSet("reccomendUs") && (!fieldIsSet("generalComments") )) {
		...
		a = new Action() {
			...
			public void act(HashMap<String, Float> semanticScores, int numChains) {
				...
				semanticScores.put("generalComments", val + .3f *(numChains*numChains));
			}
		};
		semanticMap.put(c,a);
		...
		c = new Condition() {
			@Override
			public boolean eval() {
				//if they would want a job with us, we really want them to fill out the comments field
				if(fieldIsSet("positionWithUs") && (!fieldIsSet("generalComments") )) {
